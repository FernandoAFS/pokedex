...
async function PokemonDetails({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { pokemon: { name, generation, hp, attack, defense, spatk, spdef, speed, type1, type2, pokemonId}, evolutionBranch } = pokemon
  ...
  return (
    <>
      ...
      {evolutionBranch && (
        <>
          <div className='text-3xl col-span-full'>
            Evolutions:
          </div>

          {evolutionBranch.map(({ pokemonId: pokeEvolId, name, type1, type2, generation }) => (
            <Link key={pokeEvolId} href={`/pokemon/${pokeEvolId}`}>
              <PokemonCard
                ...
                {...(pokeEvolId === pokemonId && { className: 'bg-orange-500' })}
              />
            </Link>
          ))}
        </>
