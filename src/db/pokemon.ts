...
export async function getAllPokemon({
  ...
  const wherePredicate = {
    ...
    ...(generationFilter && generationFilter.length > 0 &&
    {
      generation: {
  ...
  return await prisma.pokemonDetails.findMany(
    {
      ...
      where: {
        ...wherePredicate,
        evolutionFamilyId: {
          in: pokemonResult.map(p => p.evolutionFamilyId).filter((s): s is string => !!s)
        }
      },
      orderBy: {
        generation: 'asc'
...
export async function getPokemonByUuid({
  ...
  function* recurse(p: PokemonType): Generator<PokemonType>{
    yield p
    const next = evolutionBranch.find(next => next.evolvedFrom?.pokemonId === p.pokemonId)
    if(!next)
      return
    yield* recurse(next)
  }
  ...
  if(sortedEvolutionBranch.length <= 0){
    console.warn(`Detected branch with no beginning at pokemon ${pokemon.pokemonId}. Returning with no sort...`)
    return {
      pokemon,
      evolutionBranch,
    }
