...
export const PokeStat = ({ name, value, }: PokeStatProps) => {
    ...
    return (
        <div className='flex flex-row w-full h-6'>
            ...
            <div className='grow'>
                <div className={`h-4 rounded-md ${color}`} style={{ width: `${value * 100 / 255}%` }} />
            </div>
        </div>
    )
}
...
export const PokeTypeTag = ({pokeType}: PokeTypeTagProps) => {
    ...
    return (
        <div className='h-8 w-24 p-2 flex justify-center align-middle text-center rounded-md text-white' style={{background: color}} >
            <div>
                {pokeType}
            </div>
        </div>
